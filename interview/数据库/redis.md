...
Redis 集群是一个可以*在多个 Redis 节点之间进行数据共享*的设施
...
**Redis 集群数据共享**：Redis 集群使用数据分片（sharding）实现， 一个* Redis 集群包含 `16384` 个哈希槽（hash slot）， 数据库中的每个键都属于这 `16384` 个哈希槽的其中一个， 集群使用公式 `CRC16(key) % 16384` 来计算键 `key` 属于哪个槽

**Redis 集群中的主从复制**： Redis 集群对节点使用主从复制功能，集群中的每个节点都有 `1` 个至 `N` 个复制品（replica）， 其中一个复制品为主节点（master）， 而其余的 `N-1` 个复制品为从节点（slave）。

**Redis 集群的一致性保证**：不保证数据的强一致性

**参考**：[集群教程](http://redisdoc.com/topic/cluster-tutorial.html#)
