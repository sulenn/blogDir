...
34. **curl**：一个功能强大的网络工具
     ...
     - **-G/--get**：以get的方式来发送数据
     - **-X/--request <command>**：指定什么方式发起 HTTP请求。curl 默认的 HTTP 动词是 GET，使用 -X 参数可以支持其他动词。
     - **-g/--globoff**：Disable URL sequences and ranges using {} and []，忽略 URL 中的 {} 和 [] 符号
    
35. **crontab 定时**：

     1. crontab的命令构成为 时间+动作，其时间有**分、时、日、月、周**五种，操作符有
         - ***** 取值范围内的所有数字
         - **/** 每过多少个数字
         - **-** 从X到Z
         - **，**散列数字
...
36. **awk**：是一种处理文本文件的语言，是一个强大的文本分析工具
     ...
     - **-F fs or --field-separator fs**：指定输入文件折分隔符，fs是一个字符串或者是一个正则表达式，如 **-F :**（以 **:** 为分隔符切割字符串

     - **NF**：分割之后的字符串数量

       ```shell
       pwd # /data/home/practice/qbzeng/tool_script
       
       pwd | awk -F'/' '{print NF}'  # 6
       pwd | awk -F'/' '{print $(NF)}'  # tool_script
       ```
     ...
     - **OFS**：输出字段分隔符，默认值与输入字段分隔符一致
       ...
       pwd | awk -F'/' '{print $(NF)}' | awk -F'_' '{OFS="_"}{print $(1)}'  # tool
       pwd | awk -F'/' '{print $(NF)}' | awk -F'_' '{OFS="_"}{print $(1),$(2)}'  # tool_script
       pwd | awk -F'/' '{print $(NF)}' | awk -F'_' '{OFS="_"}{print $(1),$(2),$(3)}'  # tool_script_
       pwd | awk -F'/' '{print $(NF)}' | awk -F'_' '{OFS="_"}{print $(1),$(2),$(3),$(4)}' #tool_script__
       ```
...
- [linux之间的文件互传-scp命令](https://www.jianshu.com/p/d8ba6c2d4c41)
