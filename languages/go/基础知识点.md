...
*默认情况下，通道是不带缓冲区的。发送端发送数据，同时又必须接收相应的接收数据。如果没有缓冲区，但是又有发送端发送数据，则会出现下面的错误（死锁）*
...
第三个用法是初始化数组时，表示不定长：nums := [...]{1,2,3,4,5,6}
...
- 如果包 1 中结构体名为大写 Struct1，则包 2 可以调用它
  ...
  *本条结论参考链接 2，注意：如果调用发生在同一个包内，不管大小写，都没关系*
...
3. *copy无法自己复制给自己，即copy(a,d)这样虽然不报错，但是没有任何效果*（没有和衍生的slice脱离关系）
...
recover 可以将 paniv 产生的异常捕获，然后将运行恢复正常，继续执行下面的程序代码

**参考**:

- [【Go语言】错误与异常处理机制](https://www.cnblogs.com/Mike-zh/p/3789664.html)
...
go tool vet更加完整，它可用于文件和目录。递归遍历目录来找到包。
...
匿名结构体就是不需要结构体名。命令结构体可以运用在复写结构体属性类型，如下。Receipt 中 Status 类型为 int，但是 json 字符串中 Status 类型为 string，这时无法使用 unmarshal 将 json 字符串转为 Receipt，于是可以选择匿名结构体，如下：
...
var str = "{\"blockHash\":\"0x877b3d840743fedb23d9d773fe99e8d3c92dc162b444e83aa357173b6667d65f\",\"blockNumber\":\"0x1\",\"from\":\"0xfbb18d54e9ee57529cda8c7c52242efe879f064f\",\"gasUsed\":\"0x9e76\",\"logs\":[],\"root\":\"0xd47bd4e0e2652a965feb7cef897836b8f3ad2bc7b6c5db2a7f9573196c974c5c\",\"status\":\"0x4\",\"to\":\"0x0000000000000000000000000000000000001001\",\"transactionHash\":\"0x9dd65a4acf900ce395e64000f0ee681004c0d21d2234ddd0b7614366db078512\",\"transactionIndex\":\"0x0\"}"

func main() {
	var receipt = &struct {   // 匿名结构体
		Receipt
		Status string `json:"status"`   // 复写 Status 类型，从 int 变为 string
	}{}
	err := json.Unmarshal([]byte(str), receipt)   // unmarshal 解的时候，JSON 字符串中的 Status 赋值给匿名结构体中的 string 类型的 Status，而 int 类型的 Status 值为0
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("%+v", *receipt)
	r := receipt.Receipt
	status, err := strconv.ParseInt(receipt.Status[2:], 16, 32)
	if err != nil {
...
![image-20200728144305556](https://i.loli.net/2020/07/28/mPgNcdVh169KX4n.png)

## Golang 中使用 JSON 的小技巧

*上一小节的内容可以在下面的链接中找到*

**参考**：[Golang 中使用 JSON 的小技巧]([https://colobu.com/2017/06/21/json-tricks-in-Go/#%E4%BD%BF%E7%94%A8_json-RawMessage](https://colobu.com/2017/06/21/json-tricks-in-Go/#使用_json-RawMessage))
...
golang中有两种类型，**静态类型（static type）**和**动态类型（dynamic type）**
...
func main()  {
    // 变量i和变量g只有静态类型，变量i的类型为int，g的类型为main.Goose，
    var i int
    var g Goose
    fmt.Printf("%T\n", i)
    fmt.Printf("%T\n", g)
